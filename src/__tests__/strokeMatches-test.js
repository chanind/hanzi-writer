...
const getStroke = (charStr, strokeNum) => {
  const charJson = require(`hanzi-writer-data/${charStr}.json`);
  const char = new CharDataParser().generateCharacter(charStr, charJson);
  return char.strokes[strokeNum];
};

const assertMatches = (charStr, strokeNum, points, isOutlineVisible = false) => {
  const stroke = getStroke(charStr, strokeNum);
  const userStroke = { points };
  expect(strokeMatches(userStroke, stroke, isOutlineVisible)).toBe(true);
};

const assertNotMatches = (charStr, strokeNum, points, isOutlineVisible = false) => {
  const stroke = getStroke(charStr, strokeNum);
  const userStroke = { points };
  expect(strokeMatches(userStroke, stroke, isOutlineVisible)).toBe(false);
};
...
describe('strokeMatches', () => {
  ...
  it('is more lenient depending on how large leniency is', () => {
    ...
    expect(strokeMatches(userStroke, stroke, true, 0.2)).toBe(false);
    expect(strokeMatches(userStroke, stroke, true, 20)).toBe(true);
  });
